...
abstract class TweetSet {
  ...
  def ascendingByRetweet: Trending = ascendingByRetweetHelper(new EmptyTrending +findMin) 
  
  def ascendingByRetweetHelper(Trend:Trending):Trending =
    if(this.isEmpty)Trend
    else remove(findMin).ascendingByRetweetHelper(Trend+findMin)
...
class Empty extends TweetSet {
  ...
  // -------------------------------------------------------------------------
  def union(that:TweetSet):TweetSet = if(!that.isEmpty)that.union(this)
  else new Empty
}
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filter0(p: Tweet => Boolean, accu: TweetSet): TweetSet = 
    if(p(elem)) tail.filter0(p,accu.incl(elem))
...
object GoogleVsApple {
  ...
  val googleTweets: TweetSet = TweetReader.allTweets.filter((tweet:Tweet)=>google.exists(tweet.text.contains))
  val allTweets:TweetSet = TweetReader.allTweets
  val appleTweets: TweetSet = TweetReader.allTweets.filter((tweet:Tweet)=>apple.exists(tweet.text.contains))
